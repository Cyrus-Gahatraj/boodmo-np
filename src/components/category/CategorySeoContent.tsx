...
export function CategorySeoContent({ categoryName }: CategorySeoContentProps) {
  const isMaintenance = categoryName.toLowerCase().includes("maintenance");

  return (
    <div className="mt-10 space-y-8 border-t border-[#e2edf7] pt-8">
      <section>
        <h2 className="mb-2 text-lg font-semibold text-[#394b63]">
          {isMaintenance ? "About car maintenance parts" : `About ${categoryName}`}
        </h2>
        <p className="text-sm leading-relaxed text-[#5a6b7d]">
          {isMaintenance 
            ? "Regular maintenance of your car ensures longevity and safety. Quality replacement parts for servicing—such as filters, belts, spark plugs, and fluids—help keep your vehicle running smoothly. Factors like driving conditions, mileage, and manufacturer guidelines determine how often parts need to be replaced."
            : `High-quality ${categoryName} are essential for your vehicle's performance and safety. Finding the right parts ensures that your car remains reliable and efficient on the road. We offer a wide range of genuine and aftermarket ${categoryName} to suit every need and budget.`
          }
        </p>
      </section>
      ...
      {!isMaintenance && (
        <section>
          ...
          <ul className="list-inside list-disc space-y-1 text-sm text-[#5a6b7d]">
            <li>Wide selection of OEM and high-quality aftermarket brands</li>
            <li>Guaranteed compatibility with your vehicle model</li>
            <li>Competitive pricing and regular deals</li>
            <li>Detailed product specifications and high-resolution images</li>
          </ul>
        </section>
      )}
